...
func (m *DiskMonitor) Sample() (result *DiskSample, err error) {
	...
	samples := metrics.FilterStorageSamples(m.samples)
	if len(samples) == 0 {
	...
	ud := getUtilizationDataFromIoCountersDelta(elapsedMs, m.ioCountersStats, m.lastDiskStats)
...
func getStorageData(samples []*storage.Sample) (sd storageData) {
	...
	for _, ss := range samples {
		...
		if errD != nil {
			log.Println("cannot obtain disk from device")
			continue
		}
